...
1. It is up to everyone in the editorial group to check if there are pull requests waiting to be approved. All PRs should be reviewed by another member and, if approved, merged by the same member, if the update isn't minor (i.e. unlikely that typo:s or wrong links have been included). SciLifeLab Data Centre has pull request guidelines that we recommend reading: https://github.com/ScilifelabDataCentre/development-guidelines/tree/main/pull_requests. 
